...
$(document).ready(function() {
    ...
    $(document).on('click', '.view-note-btn', function() {
        ...
        // AJAX request to fetch the full note content (replace with your actual API endpoint)
        $.ajax({
            url: 'http://localhost/api/V1/notes/read.php?note_id=' + noteId, // Your API endpoint to fetch a note by ID
            type: 'GET',
            headers: {
                'Authorization': 'Bearer ' + 'b9214b682dd2de9e695083ffc51b71302cac6a1aa52d740c84ff1122a305e534' // Include the access token if needed
            },
            success: function(response) {
                // Update the modal with the note title and full content
                $('#note-title').text(response.note.title);
                $('#note-content').text(response.note.content); 
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.edit-note-btn', function () {
                ...
                // 1. AJAX request to fetch the note data
                $.ajax({
                    ...
                    success: function (response) {
                        // Access the note data directly from response.note
                        if (response.note) {
                            $('#addEditNotesModalLabel').text("Edit Note");
                            $('#note-id').val(response.note.id);
                            $('#note-title-input').val(response.note.title);
                            $('#note-description-input').val(response.note.content);
                            $('#update-note').show(); // Show the update button
            ...
            $('#update-note').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        //Optionally, you can also reload the notes to reflect the changes:
                        location.reload(); 
                    },
                    error: function (xhr, status, error) {
